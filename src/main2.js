let cameras = [
    {
        ...
        height: 512,
        position: [
            -0.01534894, -0.00555787, 1.5869375,
        ],
        rotation: [
            [0.9992857, -0.02804374, 0.02533019],
            [-0.02880082, -0.9991339, 0.03003524],
            [0.02446595, -0.03074332, -0.9992279],
        ],
        fy: 2376,
        fx: 2376,
    },
...
function createWorker(self) {
    ...
    function generateTexture() {
        ...
        for (let i = 0; i < vertexCount; i++) {
            ...
            //start = 0;
            for (let j = 0; j < 51; j++) {
                texdata_f[8 * i + 0] = texdata_f[8 * i + 0] +  blend[start][j + 1] * f_buffer[f_length * i + 8 + j * 3 + 0];
                texdata_f[8 * i + 1] = texdata_f[8 * i + 1] +  blend[start][j + 1] * f_buffer[f_length * i + 8 + j * 3 + 1];
                texdata_f[8 * i + 2] = texdata_f[8 * i + 2] +  blend[start][j + 1] * f_buffer[f_length * i + 8 + j * 3 + 2];
            }
    ...
    function runSort(viewProj) {
        ...
        for (let i = 0; i < vertexCount; i++) {
            let depth =
                ((viewProj[2] * f_buffer[f_length * i + 0] +
                    viewProj[6] * f_buffer[f_length * i + 1] +
                    viewProj[10] * f_buffer[f_length * i + 2]) *
                    4096) |
                0;
            sizeList[i] = depth;
...
async function loadSplatFile() {
    try {
        const response = await fetch('yourfile.splat');
        if (!response.ok) {
            throw new Error('Failed to load .splat file: ' + response.status);
        }

        const splatData = await response.arrayBuffer();
        console.log('Loaded .splat file:', splatData);

        // You can now process the binary data here...
    } catch (error) {
...
async function main() {
    ...
    async function fetchBlendData() {
        try {
            //const response = await fetch('blend.json');
            const response = await fetch('train_blend.json');
            const data = await response.json();
            blendData = data.map(row => new Float32Array(row)); // Convert each row to Float32Array
            console.log(blendData);  // Log after fetching and processing the data

            worker.postMessage({ blend: blendData });
    ...
    //     .then(data => {
    //         const blendData = data.map(row => new Float32Array(row));
    //     }
    ...
    // // Read the .npy file from the file system
    // fs.readFile('src/blend.npy', (err, data) => {
    //     if (err) {
    //         console.error('Error reading the .npy file:', err);
    //         return;
    //     }

    //     // Parse the .npy file content
    //     const arrayData = npy.parse(data);

    //     // Access the parsed data (e.g., the ndarray)
    //     console.log('Parsed .npy file data:', arrayData.data);
    //     console.log('Shape of the array:', arrayData.shape);
    //     console.log('Data type:', arrayData.dtype);

    //     // Now you can use the parsed array data in your application
    // });
    ...
    console.log("InnerHieght", innerHeight, downsample)
